...
| 34 | `feat/request-validation` | **Security hardening branch 5 of 6 (final backend branch).** Validates two kinds of untrusted request input that previously reached Mongo unchecked. New `src/utils/validate.js` exports `isValidObjectId(id)` (wraps `mongoose.Types.ObjectId.isValid`) and `parseDate(value)` (returns a `Date` or `null` for missing/invalid input). **(1) ObjectId** — `updateCategory` / `deleteCategory` in `categoryController.js` passed a raw `:id` param straight to `findByIdAndUpdate` / `findByIdAndDelete`; a malformed id threw a cast error → `500`. Now an early `isValidObjectId` check returns `400 Invalid Category ID`. (Other controllers — RF, notifications, voucher — already had this check; categories was the gap.) **(2) Date query params** — `getAllTithes`, `getAllRequestForms`, and all six report endpoints (`getTithesReport`, `getExpenseReport`, `exportTithes/ExpenseExcel/PDF`) fed `?startDate` / `?endDate` directly into `new Date(...)`; a bad value produced an `Invalid Date` and a silently-broken or erroring filter. Each now uses `parseDate` and returns `400 Invalid startDate or endDate` when either is unparseable. Pure input-validation, additive — valid requests behave exactly as before. |
